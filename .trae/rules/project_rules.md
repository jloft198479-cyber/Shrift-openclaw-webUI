...
- 异常级：网络断开、Gateway 离线、文件不存在、数据为空——是否都有合理降级？
...
- 事件契约：事件名称和 payload 结构必须定义清晰——`phase` vs `reason` 的一字之差曾导致子 Agent 结果永不送达
...
- 错误边界：一个组件崩溃不应拖垮整个页面
- **Sync 模式就是退化安全的典范**：不依赖 Gateway 事件字段（脆弱），改用文件对账（确定）
