...
sessionSync.init(_broadcastSSE);

function _onSubagentGatewayEvent(data) {
  sessionSync.onSubagentGatewayEvent(data);
}
...
function handleSubagentStop(req, res) {
  collectBody(req, function (body) {
    if (!body || !body.childSessionKey) {
      res.writeHead(400, { 'Content-Type': 'application/json' });
      res.end(JSON.stringify({ error: 'Missing childSessionKey' }));
      return;
    }
    if (!wsClient || !wsClient.isConnected()) {
      res.writeHead(503, { 'Content-Type': 'application/json' });
      res.end(JSON.stringify({ error: 'Gateway WS not connected' }));
      return;
    }
    wsClient.sendRequest('sessions.delete', {
      key: body.childSessionKey,
...
function handleSubagentStatus(req, res) {
  if (!wsClient || !wsClient.isConnected()) {
    res.writeHead(503, { 'Content-Type': 'application/json' });
    res.end(JSON.stringify({ error: 'Gateway WS not connected' }));
    return;
  }
  var sessionKey = new URL(req.url, 'http://localhost').searchParams.get('sessionKey');
  if (!sessionKey) {
    res.writeHead(400, { 'Content-Type': 'application/json' });
    res.end(JSON.stringify({ error: 'Missing sessionKey param' }));
    return;
  }
  wsClient.sendRequest('sessions.list', { prefix: sessionKey }, 10000).then(function (result) {
    res.writeHead(200, { 'Content-Type': 'application/json' });
...
function handleChatSend(req, res) {
  collectBody(req, function (body, raw, err) {
    ...
    // Wait for chat.send response to get runId, then return to client.
    // Actual streaming content arrives via SSE chat events.
    // Safety timeout in case Gateway doesn't respond promptly.
    ...
    wsClient.chatSend(sessionKey, body.message).then(function (result) {
      ...
      console.log('[ChatSend] Message sent, runId:', reply.runId || 'n/a');
...
const ROUTES = [
  ...
  { method: 'POST',   pattern: /^\/api\/subagent\/stop$/,            handler: function (m, req, res) { handleSubagentStop(req, res); } },
  { method: 'GET',    pattern: /^\/api\/subagent\/status$/,          handler: function (m, req, res) { handleSubagentStatus(req, res); } },
  ...
  { method: 'POST', pattern: /^\/api\/sessions\/sync-start$/,          handler: function (m, req, res) { collectBody(req, function (b, _r, err) { if (err) { _jsonRes(res, 500, {error:err.message}); return; } try { sessionSync.stopSync(); sessionSync.startSync(); _jsonRes(res, 200, { success: true }); } catch (e) { _jsonRes(res, 500, { error: e.message }); } }); } },
];
