const Api = {
  ...
  chat: async function (messages, agentId, callbacks) {
    ...
    try {
      ...
      if (!isSSE) {
        ...
        if (json.model && json.model.indexOf('openclaw:') === 0) {
          nonStreamAgent = json.model.slice(9) || nonStreamAgent;
        }
        ...
        for (let i = 0; i < lines.length; i++) {
          ...
          if (line.startsWith('data: ')) {
            ...
            try {
              ...
              if (json.model && json.model.indexOf('openclaw:') === 0) {
                const switchedAgent = json.model.slice(9);
  ...
  stopSubagent: async function (childSessionKey) {
    return this._fetch('/api/subagent/stop', {
      method: 'POST',
      body: JSON.stringify({ childSessionKey: childSessionKey }),
    });
  },

  getSubagentStatus: async function (sessionKey) {
    return this._fetch('/api/subagent/status?sessionKey=' + encodeURIComponent(sessionKey));
  },

  syncSession: async function (sessionId) {
    return this._fetch('/api/sessions/' + encodeURIComponent(sessionId) + '/sync', { method: 'GET' });
  },

  startSync: async function () {
    return this._fetch('/api/sessions/sync-start', {
      method: 'POST',
      body: JSON.stringify({}),
    });
  },

  chatSend: async function (sessionKey, message, callbacks) {
    var onDelta = callbacks.onDelta || function () {};
    ...
    try {
      var res = await fetch('/api/chat/send', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ sessionKey: sessionKey, message: message }),
      });

      if (!res.ok) {
        var errText = '';
        try { errText = await res.text(); } catch (e) {}
        throw new Error('chatSend HTTP ' + res.status + (errText ? ': ' + errText.slice(0, 200) : ''));
      }
